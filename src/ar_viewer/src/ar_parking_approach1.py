...
while not rospy.is_shutdown():
    ...
    #added
    if int(distance) >= 50 and int(distance) < 73:
	    xycar_msg.data = [0, 0]
	    motor_pub.publish(xycar_msg)
	    print(arData["DX"])
    elif arData["DX"] > 0 and round(yaw,1) < 0.0:
        xycar_msg.data = [30, speed]
        motor_pub.publish(xycar_msg)
        print(arData["DX"])
    elif arData["DX"] > 0 and round(yaw,1) > 0.0:
        if round(yaw,1) > 3:
            xycar_msg.data = [10, speed]
            #motor_pub.publish(xycar_msg)
        motor_pub.publish(xycar_msg)
        print(arData["DX"])
    elif arData["DX"] < 0 and round(yaw,1) < 0.0:
        xycar_msg.data = [-20, speed]
        #if round(yaw,1) < 15:
            #xycar_msg.data = [-20, speed]
        motor_pub.publish(xycar_msg)
        print(arData["DX"])
        ...
        xycar_msg.data = [-30, speed]
        #if int(distance) >= 50 and int(distance) < 70:
	        #xycar_msg.data = [0, 0]
	        #motor_pub.publish(xycar_msg)
        motor_pub.publish(xycar_msg)
        print(arData["DX"])
